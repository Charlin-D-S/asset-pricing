...
with left_col:
    ...
    maturity = st.number_input("Maturity (years)", min_value=0.01, value=1.0, format="%.2f")
    if ticker:
    ...
    if yield_curve is not None:
        ...
        r_guess = 0.01
    rate = st.number_input("Risk-free rate (r)", value=float(r_guess), format="%.2f")
...
if compute:
    ...
    try:
        if model_choice.startswith("Black"):
            bs = get_black_scholes_model(spot=spot, rate=rate/100, dividend_yield=dividend_yield,vol=vol)
            ...
            col4.metric("Rate used", f"{rate:.2f}")

        else:
            # Monte Carlo
            mc = get_monte_carlo_model(spot=spot, rate=rate/100, vol=vol, dividend_yield=dividend_yield)
